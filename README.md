...
├── propintel_ai_app.py                  # PropIntel AI app for Bengaluru and Mysuru
├── propintel_engine.py                  # Multi-agent intelligence pipeline
├── data/
│   └── bengaluru_mysuru_inventory.csv   # Seed inventory and market data
...
This folder now includes a dedicated app called **PropIntel AI - Bengaluru & Mysuru Real Estate Intelligence Platform**.

### What PropIntel AI does

- Runs a multi-agent intelligence workflow over Bengaluru and Mysuru data:
  - Data Agent: Loads and validates inventory
  - Locality Scout Agent: Applies filters and locality ranking
  - Market Pulse Agent: Computes city-level market metrics
  - Risk Watch Agent: Flags legal and climate risk exposure
  - Investment Advisor Agent: Produces risk-adjusted investment scores
- Supports interactive filtering by:
  ...
  - Risk tolerance
- Shows recommendation tables, locality intelligence, and market charts.
...
2. Start the PropIntel app:

```bash
streamlit run propintel_ai_app.py
```

No external API key is required for this PropIntel app because it runs on the included Bengaluru and Mysuru dataset.
